...
int main(void) {
	...
	xTaskCreate(TaskUartManager, (const char *) "TASK1",
	configMINIMAL_STACK_SIZE * 10,
	NULL,
	tskIDLE_PRIORITY + 4, &xTask_uart);

	xTaskCreate(TaskSpi0Master, (const char *) "TASK2",
	configMINIMAL_STACK_SIZE * 10,
	NULL,
	tskIDLE_PRIORITY + 3, &xTask_spi0);

	xTaskCreate(TaskSpi1Slave, (const char *) "TASK3",
	configMINIMAL_STACK_SIZE * 10,
	NULL,
	...
	while (1) {
		...
		if (flag == 1) { // flag is set to 1 when the user enters the end sequence on SPI0-SPI1 mode
			...
			while (XUartPs_IsReceiveData(XPAR_XUARTPS_0_BASEADDR)) {
				RecvChar_1 = RecvChar;
				RecvChar = XUartPs_ReadReg(XPAR_XUARTPS_0_BASEADDR,
						XUARTPS_FIFO_OFFSET);
...
static void TaskSpi0Master(void *pvParameters) {
	...
	while (1) {
		xQueueReceive(xQueue_FIFO1, &task2_receive_from_FIFO1, //queue to receive the data from UART Manager task
				portMAX_DELAY);
		...
		if (spi_master_loopback_en == 1 && current_command_execution_flag == 2) //just send the characters back to task 1. No SPI access! This is because loopback is currently enabled.
			xQueueSendToBack(xQueue_FIFO2, &task2_receive_from_FIFO1, 0UL);
				...
				&& (current_command_execution_flag == 2)) { //if global variable spi_master_loopback_en=0, we enable SPI connection mode now!
			/*******************************************/
