...
int positionSequence[SEQUENCE_LENGTH][2] = { {
...
int main(void) {
	...
	NULL,
	tskIDLE_PRIORITY + 1, &xUarttask);

	xTaskCreate(_Task_Motor, (const char *) "Motor Task",
	configMINIMAL_STACK_SIZE * 10,
	NULL,
	tskIDLE_PRIORITY + 2, &xMotortask);

	xTaskCreate(_Task_Emerg_Stop, (const char *) "Emergency Stop Task",
	configMINIMAL_STACK_SIZE,
	NULL,
	tskIDLE_PRIORITY + 3, &xEmergStopTask);
	...
	while (1) {
		...
		if (message_flag == 0) {
			if (parameters_flag == 0) {
				...
				xil_printf(
						"Current dwell time of the motor between movements = %i ms\n",
		...
		while (1) {
			...
			if (XUartPs_IsReceiveData(XPAR_XUARTPS_0_BASEADDR)) {
				read_UART_character[idx] = XUartPs_ReadReg(
						XPAR_XUARTPS_0_BASEADDR, XUARTPS_FIFO_OFFSET);
				idx++;
		...
		if (invalid_input_flag == 1) {
			message_flag = 1;
			xil_printf(
					"There was an invalid input from user except the valid inputs between 0-9\n");
		...
		// if we are still getting motor parameters from the user
			...
			if (parameters_flag == 0) {
				if (keep_default_value_flag == 1) {
					xil_printf(
							"User chooses to keep the default value of current position = %d steps\n\n",
							motor_parameters.currentposition_in_steps);
				} else {
					motor_parameters.currentposition_in_steps = atoi(
							str_value_motor_value);
					xil_printf(
							"User entered the new current position = %d steps\n\n",
							motor_parameters.currentposition_in_steps);
				}
			}
			// update the value with the one given by the user, or keep it as the default
				...
				if (keep_default_value_flag == 1) {
					printf(
							"User chooses to keep the default value of rotational speed = %0.1f steps/sec\n\n",
							motor_parameters.rotational_speed);
				} else {
					motor_parameters.rotational_speed = atoi(
							str_value_motor_value);
					printf(
							"User entered the new rotational speed = %0.1f steps/sec\n\n",
							motor_parameters.rotational_speed);
				}
			}
			// update the value with the one given by the user, or keep it as the default
				...
				if (keep_default_value_flag == 1) {
					printf(
							"User chooses to keep the default value of rotational acceleration = %0.1f steps/sec/sec\n\n",
							motor_parameters.rotational_acceleration);
				} else {
					motor_parameters.rotational_acceleration = atoi(
							str_value_motor_value);
					printf(
							"User entered the new rotational acceleration = %0.1f steps/sec/sec\n\n",
							motor_parameters.rotational_acceleration);
				}
			}
			// update the value with the one given by the user, or keep it as the default
				...
				if (keep_default_value_flag == 1) {
					printf(
							"User chooses to keep the default value of rotational deceleration = %0.1f steps/sec/sec\n\n",
							motor_parameters.rotational_deceleration);
				} else {
					motor_parameters.rotational_deceleration = atoi(
							str_value_motor_value);
					printf(
							"User entered the new rotational deceleration = %0.1f steps/sec/sec\n\n",
							motor_parameters.rotational_deceleration);
				}
			}
			// update the value with the one given by the user, or keep it as the default
				...
				if (keep_default_value_flag == 1) {
					xil_printf(
							"User chooses to keep the default value of destination position = %d\n\n",
							positionSequence[sequenceIndex][0]);
				} else {
					positionSequence[sequenceIndex][0] = atoi(
							str_value_motor_value);
					xil_printf(
							"User entered the new destination position = %d steps\n\n",
							positionSequence[sequenceIndex][0]);
				}
			}
			// update the value with the one given by the user, or keep it as the default
				...
				if (keep_default_value_flag == 1) {
					xil_printf(
							"User chooses to keep the default value of dwell time = %d\n",
							positionSequence[sequenceIndex][1]);
				} else {
					positionSequence[sequenceIndex][1] = atoi(
							str_value_motor_value);
				...
				xil_printf(
						"*** Pair: %d\t, <destination, delay> = <%d, %d>\n\n",
						sequenceIndex + 1, positionSequence[sequenceIndex][0],
						positionSequence[sequenceIndex][1]);
			...
			// once we have collected all of the motor parameters
				...
				if (keep_default_value_flag
						== 1|| sequenceIndex == SEQUENCE_LENGTH) {
					xil_printf(
					...
					while (1) {
						...
						if (XUartPs_IsReceiveData(XPAR_XUARTPS_0_BASEADDR)) {
							command_1_or_2_values[index] = XUartPs_ReadReg(
					...
					// start the sequence if the user enters 'g'
						...
						loop_count = 1;
						decision_parameters * const pointer_to_motor_struct_values =
								&motor_parameters;
						xQueueSendToBack(xQueue_FIFO1,
								&pointer_to_motor_struct_values, 0UL);
					}
				}
...
static void _Task_Motor(void *pvParameters) {
	...
	while (1) {
		...
		xQueueReceive(xQueue_FIFO1, &read_motor_parameters_from_queue,
				portMAX_DELAY);
...
static void _Task_Emerg_Stop(void *pvParameters) {
	...
	const TickType_t xFlashDelay = pdMS_TO_TICKS(500UL);
	...
	while (1) {
		...
		if (pressedCount >= 3) {
			...
			vTaskSuspend(xMotortask);
			vTaskSuspend(xUarttask);

			sequenceIndex = 0;
			xil_printf("Reset the board to continue.\n");
			while (1) {
				XGpio_DiscreteWrite(&Red_RGBInst, 1, RED_IN_RGB);
				vTaskDelay(xFlashDelay);
				XGpio_DiscreteWrite(&Red_RGBInst, 1, OFF_IN_RGB);
				vTaskDelay(xFlashDelay);
			}
