...
#define CHAR_PERCENT			0x25	/* '%' character is used in the restart sequence */
#define CASE_BIT 				(1 << 5) /* Bit to toggle to change cases in an ASCII character */
...
int main() {
	...
	xTaskCreate(Task_UART_buffer_send, "uart_transmit_task", 1024, (void*) 0,
	tskIDLE_PRIORITY, &task_transmituarthandle);
...
void Task_UART_buffer_receive(void *p) {
	...
	printString(
			"To display the status messages, enter the end-of-block sequence, \\r#\\r and then press ENTER\n");
	printString(
			"To reset the ISR transmit and receive global counters as well as byte counter, enter the sequence \\r%\\r and then press ENTER\n");
	...
	for (;;) {
		while (1) {
			...
			if (MyIsReceiveData() == pdFALSE || MyIsTransmitFull() == pdTRUE)
				continue;
			...
			if ((write_to_queue_value >= 'a' && write_to_queue_value <= 'z') || (write_to_queue_value >= 'A' && write_to_queue_value <= 'Z')) {
				write_to_queue_value = write_to_queue_value ^ CASE_BIT;
			}
			...
			if (restartFlag == 2 && write_to_queue_value == CHAR_CARRIAGE_RETURN) {
				restartFlag = 0;
				Countbytes = CountRxIrq = CountTxIrq = 0;
				printString(
						"Byte Counter, CountRxIrq && CountTxIrq set to zero\n\n");
			} else if (restartFlag == 1 && write_to_queue_value == CHAR_PERCENT) {
				restartFlag = 2;
			} else if (write_to_queue_value == CHAR_CARRIAGE_RETURN) {
				restartFlag = 1;
			} else {
