...
int SetupInterruptSystem(INTC *IntcInstancePtr, XUartPs *UartInstancePtr, u16 UartIntrId) {
    ...
    // The interrupt handler which handles the interrupts for the UART peripheral is connected to
    // it's unique ID number (82 in this case)
    Status =
        XScuGic_Connect(IntcInstancePtr, UartIntrId, (Xil_ExceptionHandler)XUartPs_InterruptHandler,
    ...
    // Create mask for UART interrupt, Enable the interrupt when the receive buffer has reached a
    // particular threshold
    IntrMask = XUARTPS_IXR_TOUT | XUARTPS_IXR_PARITY | XUARTPS_IXR_FRAMING | XUARTPS_IXR_OVER |
               XUARTPS_IXR_TXEMPTY | XUARTPS_IXR_RXFULL | XUARTPS_IXR_RXOVR;
